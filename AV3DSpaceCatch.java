...
public class AV3DSpaceCatch extends JComponent
    ...
    public void DesenharEspaco(AV3DSpaceCatch comp)
        ...
        for (int i = 0; i < EspacoLinhas.length; i++)
            ...
            if (((Phi % Math.PI < 4 * Math.PI / 9) || (Phi % Math.PI > 5 * Math.PI / 9)) || (((((Phi % (2 * Math.PI) < Math.PI) && (zo - z < 0))) || (((Phi % (2 * Math.PI) > Math.PI) && (zo - z > 0)) || ((Phi % (2 * Math.PI) < Math.PI) && (zo - z < 0))) && (Math.abs(Math.acos(ProdutoEscalaro / Math.sqrt(xo * xo + yo * yo + zo * zo))) < AnguloVisao + MargemAnguloVisao)) && ((((Phi % (2 * Math.PI) < Math.PI) && (zd - z < 0))) || ((Phi % (2 * Math.PI) > Math.PI) && (zd - z > 0)) || ((Phi % (2 * Math.PI) < Math.PI) && (zo - z < 0))) && (Math.abs(Math.acos(ProdutoEscalard / Math.sqrt(xd * xd + yd * yd + zd * zd))) < AnguloVisao + MargemAnguloVisao) && (Math.min(xi, Math.min(yi, Math.min(xf, yf))) > 0) && (Math.max(xi + CorrecaoX, xf + CorrecaoX) < TamanhoPlanoX) && (Math.max(yi + CorrecaoY, yf + CorrecaoY) < TamanhoPlanoY)))
